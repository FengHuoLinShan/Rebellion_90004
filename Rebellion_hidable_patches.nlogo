...
patches-own [
  ...
  hide                ; if hide is True, then rebeled person on this patch can avoid from cops' arrest
]
...
to setup
  ...
  ask patches [
    ...
    ; default hide value = false
    set hide false
  ]


  ; set less than 10% of patch to hide = true
  let hide-patches n-of (0.6 * count patches) patches
  ask hide-patches [
    set hide true
    set pcolor yellow  ; patch with hide = true is brown in color
  ]
...
to enforce
  ; only visible-rebels can be arrested
  let visible-rebels (agents-on neighborhood) with [ active? and [hide] of patch-here = false ]
  if any? visible-rebels [
    let suspect one-of visible-rebels
    move-to suspect
    ask suspect [
      set active? false
