...
globals [
  ...
  arrest-success-prob ; the prob of a successful enforce
]
...
to enforce
  if any? (agents-on neighborhood) with [ active? ] [
    ...
    ; arrest attempt based on success probability
    if random-float 1 < arrest-success-prob [
      ask suspect [
        set active? false
        set jail-term random max-jail-term
      ]
    ]
    ; else: arrest failed, do nothing
  ]
end
